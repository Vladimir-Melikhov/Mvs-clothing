        ...
        for item_data in items_data:
            ...
            if variant_id:
                try:
                    ...
                    raise NotFoundError(
                        f"Product variant with id {variant_id} not found")

                if variant.stock_quantity < quantity:
                    raise ValidationError(f"Insufficient stock for {product.name} - {variant.size}/{variant.color}")
                if price != variant.final_price:
                    raise ValidationError(f"Price mismatch for {product.name} - {variant.size}/{variant.color}")
            else:
